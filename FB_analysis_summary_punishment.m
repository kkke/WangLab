...
clear; close all; clc
fb = fb_extract_doric;
load("Fentanyl_Punishment_analysis_110624.mat")
...
for i = 1:length(groupdata)
    [psth_time,groupdata(i).psth_infusion, fig] = fb.psth_fb(groupdata(i).signal, groupdata(i).time,groupdata(i).(events{k}), pre, post, plot_tf, events{k});
...
resistance_score = [];
for i = 1:length(groupdata)
    reward_punishment(i) = groupdata(i).reward.infusion_punishment;
    resistance_score(i) = groupdata(i).cluster.resistence;
end

[B, I]  = sort(reward_punishment, 'descend');
...
data.punishment.cluster = cluster;
data.punishment.reinstatement = resistance_score;
...
set(gcf,'position',[100,100,300,300])
%% regression resp vs infusin counts
mdl = fb.correlaiton_analysis_cluster(resp, reward_punishment, cluster)
ylim([0, 120])
xlim([-1.5, 3.5])
ylabel('Infusion #')

mdl = fb.correlaiton_analysis_cluster(resp2, reward_punishment, cluster)
ylim([0, 120])
xlim([-1, 3])
ylabel('Infusion #')

%% regression resp vs reistance_score
mdl = fb.correlaiton_analysis_cluster(resp, resistance_score, cluster)
ylim([0, 0.8])
xlim([-1.5, 3.5])
ylabel('Resistance Score')

mdl = fb.correlaiton_analysis_cluster(resp2, resistance_score, cluster)
ylim([0, 0.8])
xlim([-1, 3])
ylabel('Resistance Score')
