function drug_punishment_plots(data_plot, data_to_plot, population_psth_avg, psth_time)
...
set(gcf, 'Color', 'w')
%% plot example mice
for i = 1:length(data_plot.high_resistance)
plot_example_mouse(psth_time, data_to_plot, data_plot.high_resistance(i))
ylim([-1, 3])
end
figure;
...
figure;
subplot(1, 3, 1)
popPSTH_cluster(recordings(1).norm_PSTH, mean(psth_time, 2), high_taker, low_taker, [1, 4])
xlim([-10, 20])
ylim([-1, 4])

subplot(1, 3, 2)
popPSTH_cluster(recordings(2).norm_PSTH, mean(psth_time, 2), high_taker, low_taker, [1, 4])
xlim([-10, 20])
ylim([-1, 4])


subplot(1, 3, 3)
popPSTH_cluster(recordings(3).norm_PSTH, mean(psth_time, 2), high_taker, low_taker, [1, 4])
...
%% get the infusion counts across triasl
...
figure
subplot(1, 2, 1)
baf.line_plot_MA_avg(data01(high_taker,1:10), data01(low_taker,1:10))
xlim([0, 4])
