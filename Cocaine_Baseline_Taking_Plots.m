...
load('/Users/kechen/MIT Dropbox/Ke Chen/Wang Lab/Manuscripts/DA_Cocaine_Fentanyl/Figures/Figure3/data/Cocaine_FB_taking_plot.mat')
...
i = 6
j = 20
t = data_to_plot(j).infusion(i);
index = find(data_to_plot(j).raw_time > t-30 & data_to_plot(j).raw_time < t+50);

plot(data_to_plot(j).raw_time(index), data_to_plot(j).raw_signal(index))
hold on
plot(data_to_plot(j).raw_time (index), data_to_plot(j).raw_reference(index))
xlim([t-30, t+50])
%
%% plot the blinking light signals
figure
hold on
index = find(data_to_plot(j).time > t-30 & data_to_plot(j).time < t+50);

plot(data_to_plot(j).time(index), data_to_plot(j).signal(index))

%

for i = 1:length(data_to_plot(j).front)

    plot([data_to_plot(j).front(i),data_to_plot(j).front(i)],  [0, 6], 'r')
end

for i = 1:length(data_to_plot(j).leverRetraction)
    plot([data_to_plot(j).leverRetraction(i),data_to_plot(j).leverRetraction(i)],  [0, 6], 'y')
end
for i = 1:length(data_to_plot(j).leverInsertion)
    plot([data_to_plot(j).leverInsertion(i),data_to_plot(j).leverInsertion(i)],  [0, 6], 'y')
end
i = 6
j = 20

plot([data_to_plot(j).infusion(i),data_to_plot(j).infusion(i)],  [0, 6], 'g')

xlim([t-30, t+50])

% add blicking light singals 
reward_time = t;

% Parameters
fs = 1/mean(diff(data_to_plot(j).time(index)));          % sampling rate (Hz)
on_time = 1.0;      % ON duration (seconds)
off_time = 0.5;     % OFF duration (seconds)
n_repeats = 13;     % number of cycles

% Samples for each phase
on_samples = round(on_time * fs);
off_samples = round(off_time * fs);

% Generate one cycle
on_segment = ones(on_samples, 1);     % TTL high
off_segment = zeros(off_samples, 1);  % TTL low

one_cycle = [off_segment; on_segment];

% Repeat 13 times
ttl_signal = repmat(one_cycle, n_repeats, 1);

% Create a time vector if you want to plot it
t_light = (0:length(ttl_signal)-1) / fs;

% Plot
figure;
plot(t+t_light, ttl_signal);
xlabel('Time (s)');
ylabel('TTL level');
title('TTL: 1 s ON, 0.5 s OFF, repeated 13 times');
ylim([-0.2 1.2]);
...
%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%
% remove the 1st mouse due to 3-hour SA acquisition
norm_population_psth_avg(:, 1) = [];
sort_index(find(sort_index==1)) =[];
...
% remove the 1st mouse due to 3-hour SA acquisition
low_taker(1) =[];
high_taker = high_taker - 1;
low_taker = low_taker - 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j = 1:length(data_plot.low_taker)
    i = data_plot.low_taker(j)
    figure;
...
for j = 1:length(data_to_plot)
    ...
    leverPress_analysis = reshape(leverPress_trial(1:3, :), [], 1);
    leverPress_analysis(isnan(leverPress_analysis)) = [];
    data_to_plot(j).leverPress_analysis = leverPress_analysis;
    [data_to_plot(j).psth_leverpress_time, data_to_plot(j).psth_leverpress, fig] = fb.psth_fb(data_to_plot(j).signal, ...
